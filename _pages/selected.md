...
**2026** From Raw Corpora to Domain Benchmarks: Automated Evaluation of LLM Domain Expertise ***under review*** [link](https://doi.org/10.48550/arXiv.2506.07658) We present a deterministic pipeline that converts raw domain corpora into completion-style benchmarks to evaluate domain-specific knowledge in large language models without relying on other LLMs or costly human annotation. By generating prompt–target pairs from domain-specific vocabulary, the approach provides a scalable, contamination-resistant, and unbiased evaluation framework that correlates with expert-curated benchmarks and enables fair comparisons between base and instruction-tuned models.
