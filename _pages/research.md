...
P.s.: Most of the time linear regression works just as well as AI for brain readouts. 
...
* SUMMARY: EEG denoising remains challenged by complex physiological and environmental artifacts, limiting signal fidelity and real-world use. We present GEDAI (Generalized Eigenvalue De-Artifacting Instrument), a leadfield-based algorithm that removes artifacts deviating from a theoretical EEG forward model. GEDAI denoises heavily corrupted data without clean references, corrects bad channels and epochs in a single step, and separates brain from noise components using the signal–noise subspace alignment index (SENSAI). Across simulations and empirical tests with strong EOG, EMG, and noise contamination, GEDAI outperformed PCA- and ICA-based methods (ASR, ICLabel, MARA), improving ERP classification and brain fingerprinting. Fast, autonomous, and robust, it can support clinical, mobile, and dry-electrode EEG, MEG denoising, and real-time BCIs. Despite current trends no AI needed for this one. 
